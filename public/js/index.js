...
function toggleSections() {
    ...
    if (tipo !== 'empresa') {
        ['empRUT', 'empNombre', 'empDireccion', 'empGiro', 'solNombre', 'solCargo', 'solTelefono', 'solCorreo'].forEach(id => {
            document.getElementById(id).value = '';
        });
        document.getElementById('trabajadoresContainer').innerHTML = '';
    }

    // Limpiar campos de particular
    if (tipo !== 'particular') {
        ['trabNombre', 'trabRut', 'trabEdad', 'trabCargo', 'trabCorreo'].forEach(id => {
            document.getElementById(id).value = '';
        });
    }

    // Agregar al menos un trabajador si es empresa
    const container = document.getElementById('trabajadoresContainer');
    if (tipo === 'empresa' && container.children.length === 0) {
        container.appendChild(createTrabajadorRow());
    }

    // Ajustar requerimiento del correo particular
...
function mostrarModalReserva(respuesta, submitBtn) {
    ...
    const modal = new bootstrap.Modal(document.getElementById('modalReservaCreada'), {
        ...
        keyboard: false     // (opcional) No se cierra con ESC
    });
    modal.show();
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (typeof data.modificacionCount !== 'undefined' && data.modificacionCount >= 1) {
                alert("Esta reserva ya fue modificada anteriormente y no puede volver a modificarse. Si necesita una nueva modificacion contactarse con atencion al cliente *569xxxxxxxxx");
                return;
            }
